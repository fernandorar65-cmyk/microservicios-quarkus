...
public record ApiResponse<T>(Instant timestamp, int status, String message, T data, Map<String, String> errors) {
    ...
    public static ApiResponse<Void> validation(Map<String, String> errors) {
        return new ApiResponse<>(Instant.now(), 400, "Validation failed", null, Map.copyOf(errors));
    }
}
